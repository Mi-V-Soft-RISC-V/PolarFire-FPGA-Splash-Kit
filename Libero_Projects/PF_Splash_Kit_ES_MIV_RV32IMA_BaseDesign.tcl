...
set Libero_project_name_CFG1 PF_Splash_Kit_ES_MIV_RV32IMA_CFG1_BaseDesign

set project_folder_name_CFG2 MIV_CFG2_BD
set project_dir_CFG2 "./$project_folder_name_CFG2"
set Libero_project_name_CFG2 PF_Splash_Kit_ES_MIV_RV32IMA_CFG2_BaseDesign
...
if {"$config" == "CFG1"} then {
	if {[file exists $project_dir_CFG1] == 1} then {
		...
		source ./import/components/IMA_CFG1/import_component_and_constraints_pf_splash_kit_rv32ima_cfg1.tcl
		save_project
        base_design_built
	}
	...
	if {[file exists $project_dir_CFG2] == 1} then {
		...
		download_required_direct_cores
		source ./import/components/IMA_CFG2/import_component_and_constraints_pf_splash_kit_rv32ima_cfg2.tcl
		save_project
        base_design_built
	}
} elseif {"$config" != ""} then {
...
if {"$design_flow_stage" == "SYNTHESIZE"} then {
	...
	if {"$config" == "CFG1"} then {
		export_prog_job \
			-job_file_name {PF_Splash_Kit_ES_MIV_RV32IMA_CFG1_BaseDesign} \
		...
		export_prog_job \
			-job_file_name {PF_Splash_Kit_ES_MIV_RV32IMA_CFG2_BaseDesign} \
